...
colors = [c for c in list(mcolors.CSS4_COLORS.values())
          if remove_low_contrast(c)]
...
def visualize_routes(filename):
    ...
    vehicles = []
    count = 1
    with open("MDVRP/dataset/" + filename) as f:
            ...
            if len(new_arr) == 7:
                ...
                customer_list.append(
                    {"id": int(new_arr[0]),
                     "x": int(new_arr[1]),
                     "y": int(new_arr[2])})
    with open("MDVRP/results/" + filename) as f:
        next(f)
        for line in f:
            new_arr = line.split()
            vehicles.append(
...
def visualize_generations(filename):
    generations = []
    fitness = []
    with open("MDVRP/generations/" + filename) as f:
        for line in f:
            arr = line.split()
